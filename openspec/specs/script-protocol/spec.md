...
- `args` (array of objects): Each arg object MUST contain `name` (string) and `prompt` (string). MAY contain `options` (array of strings) for constrained choices.
...
- **WHEN** the CLI invokes a script with `--agrr-meta` and the returned JSON is missing a required field
